...
def main(args=None):
    ...
    if parsed_args.future_date:
        try:
            future_date = datetime.strptime(parsed_args.future_date, '%d/%m/%Y').strftime('%Y-%m-%d')
            future_date_for_function = datetime.strptime(parsed_args.future_date, '%d/%m/%Y').strftime('%d-%m-%Y')
        except ValueError:
    ...
    if parsed_args.excel_file:
        # Validate excel_dir and excel_file with security checks
        try:
            excel_dir_path = validate_directory_path(parsed_args.excel_dir, must_exist=True, logger=logger)
            excel_file_str = os.path.join(str(excel_dir_path), parsed_args.excel_file)
    ...
    try:
        if os.path.isabs(parsed_args.data_file):
            data_file_str = parsed_args.data_file
        else:
            data_file_str = os.path.join(SCRIPT_DIR, parsed_args.data_file)
    ...
    for model_name, model in models.items():
        ...
        write_predictions(predicted_df, output_path, logger=logger, skip_confirmation=parsed_args.skip_confirmation)
